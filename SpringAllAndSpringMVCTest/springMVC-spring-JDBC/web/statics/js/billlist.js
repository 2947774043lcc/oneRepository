	...
	$.ajax({
		...
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				...
				changeDLGContent("对不起，删除订单【"+obj.attr("productName")+"】失败");
			}else if(data.delResult == "notexist"){
				//alert("对不起，订单【"+obj.attr("billcc")+"】不存在");
				changeDLGContent("对不起，订单【"+obj.attr("productName")+"】不存在");
			}
		},
		error:function(data){
			alert("对不起，删除失败");
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/bill/updatebill?billid="+ obj.attr("billid");
	});
